...
class MaterialController extends HomeController {
	...
	function material_data() {
		...
		$field = 'id,title,cover_id,intro,group_id';
		$list = M ( 'material_news' )->where ( $map )->field ( $field . ',count(id) as count' )->group ( 'group_id' )->order ( 'group_id desc' )->select ();
	...
	function syc_news_to_wechat() {
		...
		$field = 'id,title,cover_id,link,intro,author,content,group_id,thumb_media_id';
		$list = M ( 'material_news' )->limit ( 10 )->where ( $map )->field ( $field . ',count(id) as count' )->group ( 'group_id' )->order ( 'group_id desc' )->select ();

		if (empty ( $list )) {
			$this->success ( '上传素材完成', U ( 'material_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
		...
		foreach ( $list as $vo ) {
			...
			if (empty ( $vo ['link'] )) {
				...
				$data ['content_source_url'] = $vo ['link'];
			}
	...
	function syc_news_from_wechat() {
		...
		if (empty ( $list ['item'] )) {
			$this->success ( '下载素材完成', U ( 'material_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
	...
	function picture_data() {
		...
		$map ['is_use'] = 1;
		$map ['token'] = get_token ();
		$data = M ( 'material_image' )->where ( $map )->field ( 'id,cover_url' )->order ( 'id desc' )->select();
		$list['list_data']=$data;
		$this->assign ( $list );
		$this->display ();
	}
	...
	function syc_image_to_wechat() {
		...
		if (empty ( $list )) {
			$this->success ( '上传素材完成', U ( 'picture_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
	...
	function syc_image_from_wechat() {
		...
		if (empty ( $list ['item'] )) {
			$this->success ( '下载素材完成', U ( 'picture_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
	...
	function voice_data() {
		...
		$map ['token'] = get_token ();
		$map ['type'] = 1;
		$data = M ( 'material_file' )->where ( $map )->order ( 'id desc' )->select();
        $list ['list_data'] = $data;
		vendor ( "getID3.getid3.getid3" );
	...
	function syc_voice_to_wechat() {
		...
		if (empty ( $list )) {
			$this->success ( '上传素材完成', U ( 'voice_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
	...
	function syc_video_to_wechat() {
		...
		if (empty ( $list )) {
			$this->success ( '上传素材完成', U ( 'video_lists', array (
					'mdm' => $_GET ['mdm']
			) ) );
			exit ();
		}
	...
	function syc_file_from_wechat() {
		...
		if (empty ( $list ['item'] )) {
			if ($type == 1) {
				$this->success ( '下载素材完成', U ( 'voice_lists', array (
						'mdm' => $_GET ['mdm']
				) ) );
				exit ();
			} else {
				$this->success ( '下载素材完成', U ( 'video_lists', array (
						'mdm' => $_GET ['mdm']
				) ) );
				exit ();
			}
	...
	function ajax_text_by_id() {
		$id = I ( 'text_id' );
		$text = M ( 'material_text' )->where(array('id'=>$id))->getField('content');
